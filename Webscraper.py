...
def makeStrategyListing(strategy, formListings):
    ...
    try:
        ...
        if misc[0][0].endswith("ite"):
            baseStats = formListings["Mega"]["base stats"]
            type = formListings["Mega"]["types"]
    except Exception as ex:
        print('->> no proper form found for the below forms with item "', misc[0], '"')
        print('\t', ex)
...
def getChecksAndCounters(article):
    return None
...
for pokemon in pokeList:
    ...
    pokePage.render(sleep=SLEEP_VALUE)
    ...
    #print(dataPart)
    #print(dataPart[2].find("div > div"))

    #print("Overview:\t", article.find("section", first=True).text)
    ...
    print("Overview:\t")
    printer.pprint(children[2].text)
    print("===========")
    ...
    printer.pprint(stratListings)




    counters = getChecksAndCounters(children[len(children)-3])

    # index len(children)-3 is "Options and Checks/Counters"
    # index len(children)-1 is credits

    credits = children[len(children)-1]

# mainPage = session.get("https://www.smogon.com/dex/sm/pokemon/")
# print("\t", mainPage.html)
# mainPage.html.render()
# print(mainPage.html.find(".DexContent", first=True).html, "\n==========================================================================\n")
#
...
#     print(link.get("href"))
#
#     if link.get("href").startswith("/dex/sm/pokemon"):
#         print("\t", link)